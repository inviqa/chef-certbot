describe 'certbot::cron' do
  context 'with default configuration' do
    ...
    it 'will create cron to renew the certificates daily' do
      expect(chef_run).to create_cron_d('certbot-renew').with(command: '/usr/local/sbin/certbot-renew.sh', user: 'root', predefined_value: '@daily')
    end
    it 'will create the cron script' do
      expect(chef_run).to render_file('/usr/local/sbin/certbot-renew.sh')
        .with_content(/certbot renew --post-hook "touch \$UPDATE_FLAG_FILE"/)
    end
  end
  ...
  context 'with specified cron timing configuration' do
    ...
    it 'will create cron to renew the certificates daily' do
      expect(chef_run).to create_cron_d('certbot-renew').with(command: '/usr/local/sbin/certbot-renew.sh', user: 'root', minute: 0, hour: 2)
    end
  end
  ...
  context 'with nginx service set in attributes to reload after renew' do
    cached(:chef_run) do
      ChefSpec::SoloRunner.new do |node|
        node.normal['certbot']['renew_post_services']['nginx'] = 'reload'
      end.converge(described_recipe)
    end
  ...
  context 'with sandbox enabled' do
    ...
    it 'will run certbot as sandbox user in the cron script' do
      expect(chef_run).to render_file('/usr/local/sbin/certbot-renew.sh').with_content(/su - certbot -c "certbot renew --post-hook \\"touch \$UPDATE_FLAG_FILE\\""/)
    end
  end
end
