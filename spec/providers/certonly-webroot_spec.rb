describe 'fake::certonly-webroot' do
  context 'with two certonly webroots' do
    ...
    it "will create a single domain certificate" do
      expect(chef_run).to run_execute('certbot certonly --config-dir /etc/letsencrypt --logs-dir /var/log/letsencrypt --work-dir /var/lib/letsencrypt --webroot --webroot-path /var/www/certbot --domains mysite1.dev --email root@localhost --rsa-key-size 2048 --agree-tos --non-interactive').with({
        user: nil
      })
    end

    it "will create a multi domain certificate" do
      expect(chef_run).to run_execute('certbot certonly --config-dir /etc/letsencrypt --logs-dir /var/log/letsencrypt --work-dir /var/lib/letsencrypt --webroot --webroot-path /var/www/certbot --domains mysite2.dev,js.mysite2.dev,css.mysite2.dev --email root@localhost --expand --rsa-key-size 2048 --agree-tos --non-interactive').with({
        user: nil
      })
    end
  end
